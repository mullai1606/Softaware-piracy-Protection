...
<%
	...
	while(rs.next()){
            %><tr style="color: black">
                ...
                <th><textarea><%=rs.getString(4)%></textarea></th>
                <th><%=rs.getString(3)%></th>
                
                 <th><%=rs.getString(5)%></th>
                <th><img height="60" width="60" src="Gallery/<%=rs.getString("image")%>"/></th>
                <td><a href="user_viewproducts1.jsp?id=<%=rs.getString("id")%>&pname=<%=rs.getString("pname")%>&pquantity=<%=rs.getString("pquantity")%>&pdes=<%=rs.getString("pdes")%>&pprice=<%=rs.getString("pprice")%>"><font color="#0d637d" size="3">Purchase</font></a> </td>
